...
export class OrdersService {
  ...
  private async createOrderTx(client: PoolClient, input: CreateOrderInput, actor: Actor) {
    ...
    const idempotent = await client.query<{ id: string }>(
      "select id from orders where client_request_id = $1::text limit 1",
      [reqId],
    );
    ...
    if (actor && addressId) {
      const addr = await client.query(
        "select id from addresses where id = $1::uuid and user_id = $2::uuid limit 1",
        [addressId, actor.id],
      );
    ...
    await client.query(
      `insert into order_events_outbox (order_id, event_type, payload)
       values ($1::uuid,'notification',jsonb_build_object('order_id',$1::uuid)),
              ...
              ($1::uuid,'analytics',jsonb_build_object('order_id',$1::uuid,'request_id',$2::text))`,
      [orderId, reqId],
    );
  ...
  private async resolvePromo(
    ...
    const promoRes = await client.query<{
      ...
      `select id, discount, discount_fixed_cents, restaurant_id, audience, min_subtotal_cents, expires_at, active
       from promocodes where code = $1::text and active = true limit 1`,
      [code],
    );
