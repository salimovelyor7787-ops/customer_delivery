...
async function pumpOutboxOnce() {
    const rows = await pgPool.query(`with picked as (
     ...
     update order_events_outbox o
      ...
      from picked p
      where o.id = p.id
     returning o.id, o.order_id, o.event_type, o.payload`, [batchSize]);
    for (const event of rows.rows) {
        await queue.add(event.event_type, { outbox_id: event.id, order_id: event.order_id, payload: event.payload }, {
...
async function markDone(id) {
    await pgPool.query(`update order_events_outbox
