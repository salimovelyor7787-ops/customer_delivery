...
export async function loadHomeCatalog(): Promise<HomeCatalogPayload> {
  ...
  const [{ data: rests }, { data: catRows }, { data: serviceRows }, { data: bannerRows }, { data: dealRows }, { data: nearbyRows }, { data: pushRows }] = await Promise.all([
    supabase.from("restaurants").select("id,name,image_url,is_open,delivery_fee_cents,category_id,category_ids").order("name", { ascending: true }).limit(20),
    supabase.from("categories").select("id,name").order("sort_order", { ascending: true }),
    supabase.from("home_service_cards").select("id,service_key,title,image_url,banner_image_url,updated_at").eq("is_active", true).order("sort_order", { ascending: true }),
    supabase
      .from("banners")
    ...
    supabase.from("home_nearby_cards").select("id,title,image_url,restaurant_id").eq("is_active", true).order("sort_order", { ascending: true }).limit(10),
